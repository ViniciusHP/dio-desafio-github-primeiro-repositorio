...
function contaCedulas(cedula, valorAtual) {
  const quantidadeDeCedulas = parseInt(valorAtual / cedula);
  console.log(`${quantidadeDeCedulas} nota(s) de R$ ${cedula},00`);
  return valorAtual - (quantidadeDeCedulas * cedula);
}

const cedulas = [100, 50, 20, 10, 5, 2, 1];
const valorLido = parseInt(gets());
let valorFaltanteContagem = valorLido;

console.log(valorLido);
for(let cedula of cedulas) {
  valorFaltanteContagem = contaCedulas(cedula, valorFaltanteContagem);
}